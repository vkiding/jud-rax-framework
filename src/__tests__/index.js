...
import {Runtime, Instance} from 'jud-vdom-tester';
import {config} from 'jud-js-runtime';
...
describe('framework', () => {
  ...
  beforeEach(() => {
    // Create a Jud JavaScript runtime for a certain Jud JS framework.
    ...
    // Create a Jud instance in a certain runtime.
  ...
  it('jud only var', () => {
    const code = `
      alert(__jud_options__.debug)
    `;

    const mockFn = jest.fn((args) => {
      expect(args).toEqual({
        message: __jud_options__.debug
      });
    });

    instance.oncall('modal', 'alert', mockFn);
    instance.$create(code, __jud_options__, __jud_data__);
  ...
  it('fetch __jud_data__', () => {
    const code = `
      ...
      }).then(function (__jud_data__) {
        console.log('fetch response __jud_data__', __jud_data__);
      });
    `;
